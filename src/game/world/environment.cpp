...
void Environment::drawBackground(Vector2 camera, float distance) const
{
    ...
    if (_fromBiome != _targetBiome && _biomeBlend < 1.0f) {
        biomeRendererFor(_fromBiome).drawHorizon(camera, day, 1.0f - _biomeBlend);
        biomeRendererFor(_targetBiome).drawHorizon(camera, day, _biomeBlend);
    } else {
    ...
    if (_fromBiome != _targetBiome && _biomeBlend < 1.0f) {
        biomeRendererFor(_fromBiome).drawAmbientParticles(camera, day, 1.0f - _biomeBlend, _seed, _time);
        biomeRendererFor(_targetBiome).drawAmbientParticles(camera, day, _biomeBlend, _seed, _time);
    } else {
        biomeRendererFor(_targetBiome).drawAmbientParticles(camera, day, 1.0f, _seed, _time);
    }
