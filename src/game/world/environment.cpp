    ...
    for (int x = start; x < kScreenWidth + step; x += step) {
        ...
        for (int px = left; px < right; px += 4) {
            ...
            if (px > crest + 4 && y < baseY - 5) {
                const float distT = game_config::clampf(
...
// void drawDistantStoneStrip(Vector2 camera, float parallax, int baseY, Color mainColor, Color topColor, Color
// crackColor)
// {
//     const float scroll = camera.x * parallax;
//     const int step = 32;
                          ...
                          float capCoverageScale = 1.0f)
{
    ...
    for (int x = start; x < kScreenWidth + step; x += step) {
        ...
        for (int px = left; px < right; px += 2) {
            ...
            const float ct = game_config::clampf(t, 0.0f, 1.0f);
            ...
            if (capCoverageScale > 0.01f) {
                ...
                if (localHeight > 6) {
                    ...
                    for (int row = 0; row < capHeight; ++row) {
                        const int yy = y + row;
                        // 覆盖比例较大（雪盖）时边界行做抖动柔化；覆盖比例较小（岩石高光）本身就薄，不需要抖动
...
void drawBiomeHorizon(Vector2 camera, float day, TerrainBiome biome, float alpha)
{
    ...
    if (biome == TerrainBiome::Stone) {
        // 复用雪地轮廓形状，去掉雪盖，换成岩石配色
        const Color unusedSnow{0, 0, 0, 0};
        drawDistantSnowStrip(
            camera,
            0.08f,
            103,
    ...
    // Mountain（默认分支）：复用沙丘轮廓形状，换成山地配色，起伏幅度调高
    constexpr float kMountainHeightScale = 1.12f;
    drawDistantDuneStrip(camera,
                         0.08f,
                         103,
